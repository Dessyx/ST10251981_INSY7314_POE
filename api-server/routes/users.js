...
router.post('/login', validateCsrfMiddleware, async (req, res) => {
  try {
    ...
    const user = await findUserByUsernameDecrypted(value.username);
    if (!user || user.account_number !== value.account_number)
...
router.get('/verify', (req, res) => {
  try {
    ...
    res.json({
      authenticated: true,
      user: { id: decoded.id, username: decoded.username, role: decoded.role || 'customer' }
    });
  } catch {
